...
class Asteroid {
public:
static void LoadAsteroidTexture() {
        asteroidTexture = LoadTexture("asteroid1.png");
        GenTextureMipmaps(&asteroidTexture);
        SetTextureFilter(asteroidTexture, TEXTURE_FILTER_BILINEAR);
        asteroidScale = 0.1f; // Dopasuj w zależności od rozmiaru grafiki
    }

    static void UnloadAsteroidTexture() {
...
class BrainrotAsteroid : public Asteroid{
public:
BrainrotAsteroid(int w, int h) : Asteroid(w, h) { baseDamage = 20; }
	void Draw() const override {
    float scale = asteroidScale * static_cast<float>(render.size); // Uwzględnij rozmiar
    Vector2 size = { asteroidTexture.width * scale, asteroidTexture.height * scale };
    Vector2 origin = { size.x / 2.0f, size.y / 2.0f };
    DrawTextureEx(asteroidTexture, 
                  Vector2Subtract(transform.position, origin), 
                  0.0f, 
                  scale, 
                  WHITE);
}
...
// Shape selector
enum class AsteroidShape { TRIANGLE = 3, SQUARE = 4, PENTAGON = 5, RANDOM = 0, BRAINROT = 6 };
...
static inline std::unique_ptr<Asteroid> MakeAsteroid(int w, int h, AsteroidShape shape) {
	...
	case AsteroidShape::BRAINROT:
		return std::make_unique<BrainrotAsteroid>(w, h);	
	default: {
...
void DrawRotatingStar(Vector2 center, int points, float outerRadius, float innerRadius, float rotationDeg, Color color) {
	const float step = PI / points; // dwa razy więcej punktów
	std::vector<Vector2> starPoints;

	for (int i = 0; i < points * 2; ++i) {
		float radius = (i % 2 == 0) ? outerRadius : innerRadius;
		float angle = rotationDeg * DEG2RAD + step * i;
		starPoints.push_back(Vector2{
	...
	for (size_t i = 0; i < starPoints.size(); ++i) {
		DrawLineV(starPoints[i], starPoints[(i + 1) % starPoints.size()], color);
	}
}

// --- PROJECTILE HIERARCHY ---
enum class WeaponType { LASER, BULLET,TRIPLE_LASER, COUNT };
class Projectile {
public:
	...
	void Draw() const {
		if (type == WeaponType::BULLET) {
			DrawRotatingStar(transform.position, 5, 10.f, 5.f, GetTime() * 360.f, LIGHTGRAY);
		}
		else {
	...
	float angleOffset = 0.f)
{
	...
	else if (wt == WeaponType::TRIPLE_LASER) {
	...
	void Run() {
		...
		while (!WindowShouldClose()) {
			...
			if (IsKeyPressed(KEY_FIVE)) {
				currentShape = AsteroidShape::BRAINROT;
			}
			...
			// Shooting
			{
				if (player->IsAlive() && IsKeyDown(KEY_SPACE)) {
					...
					while (shotTimer >= interval) {
						...
						if (currentWeapon == WeaponType::TRIPLE_LASER) {
			...
			// Render everything
			{
				...
				case WeaponType::TRIPLE_LASER: weaponName = "TRIPLE LASER"; break;
				}

				DrawText(TextFormat("Weapon: %s", weaponName), 10, 40, 20, BLUE);

				for (const auto& projPtr : projectiles) projPtr.Draw();
				for (const auto& astPtr : asteroids) astPtr->Draw();
		...
		Asteroid::UnloadAsteroidTexture();
		CloseWindow();
	}
...
Texture2D backgroundTexture{};
	...
	void DrawHPBar(const Ship& ship) {
		float hpPerc = static_cast<float>(ship.GetHP()) / 100.f;
		float barWidth = 60.f;
		float barHeight = 8.f;
		Vector2 pos = ship.GetPosition();
		Rectangle back = { pos.x - barWidth / 2, pos.y - ship.GetRadius() - 20, barWidth, barHeight };
		Rectangle front = { back.x, back.y, barWidth * hpPerc, barHeight };
		DrawRectangleRec(back, DARKGRAY);
		DrawRectangleRec(front, (hpPerc > 0.5f) ? GREEN : ((hpPerc > 0.2f) ? ORANGE : RED));
	...
	void DrawGameOver() {
		...
		DrawText(msg,
			(Renderer::Instance().Width() - textWidth) / 2,
			Renderer::Instance().Height() / 2 - fontSize,
			fontSize,
			RED);
	}
	...
	static constexpr int C_WIDTH = 1920;
	static constexpr int C_HEIGHT = 1060;
...
Texture2D Asteroid::asteroidTexture{};
float Asteroid::asteroidScale = 1.0f;
